            ...
            compareCommits: jest
              ...
              .mockImplementation(
                ({ base, head }: { base: string; head: string }) => {
                  // When comparing tags against main HEAD (mainsha)
                  // v2.0.0 is "behind" (not reachable), v1.0.0 is "ahead" (reachable)
...
describe("runAction", () => {
  ...
  it("groups PRs by author and category and outputs body", async () => {
    ...
    expect(body).toContain(
      '<img src="https://avatars.githubusercontent.com/u/1?s=32" width="20" height="20">',
    );
